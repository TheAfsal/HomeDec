...
const DetailPage = () => {
  ...
  const { isPending, error, data } = useQuery({
    ...
    queryFn: () => fetchDetails(productId), // Return the promise directly
  });

  // Handle side effects when query data is available
  useEffect(() => {
    if (data) {
      setProduct(data.product);
      setBestOffer(data.bestOffer);
      setSelectedVariant(data.product.variants[0]);
      setSelectedImage(data.product.variants[0]?.images[0]);
      setLoading(false);
    }
  }, [data]);

  // Handle loading and error states
  if (isPending || loading) return <CircularLoader />;
  ...
  const updateQuantity = async () => {
    ...
    if (!role || role !== 'user') {
      ...
      return toast.error("Please log in to buy products", {
        ...
        <div className="flex flex-col w-1/2 justify-between">
          <div className="flex flex-col gap-y-5 text-green_700">
            ...
            <p className="text-gray-500 text-sm">
              <Link to={`/${USER_ROUTES.SHOP}`}>
                <span className="hover:underline cursor-pointer text-green_700">
          ...
          <p className="text-sm font-semibold max-w-96">{product.description}</p>

          <div className="flex space-x-3">
            {product.variants?.map((variant, index) => (
              <div
                key={index}
                className={`${selectedVariant?.colorHex === variant.colorHex ? 'bg-slate-300' : ''
              ...
              >
                <button
                  ...
                  className={`w-6 h-6 rounded-full shadow-gray-500`}
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              className="p-2 min-w-36 h-10 bg-green_600 text-sm text-white rounded-md"
          ...
          <div className="text-gray-600 space-y-2">
            ...
            <div className="flex items-center space-x-2 text-md">
              <button
                className="flex items-center text-green_500 my-2 hover:text-green_700"
          ...
          <div className="flex gap-3 justify-start mt-3">
            {selectedVariant?.images?.map((url, index) => (
              <img
                ...
                className={`w-20 h-20 object-cover border-2 ${selectedImage?.secure_url === url.secure_url ? 'border-blue-500' : 'border-gray-200'
                  }`}
      ...
      <div className="mt-10">
        <div className="mb-8">
          ...
          {product?.itemProperties?.map((property, index) => (
            <p key={index} className="text-xs my-2">
              <span className="font-bold">{property.field}:</span> {property.value}
            </p>
          ))}
        </div>
        ...
        <div className="mb-6">
          ...
          >
            ...
            <div>{isExpanded.productInfo ? '-' : '+'}</div>
          </h3>
          <hr className="my-2" />
        ...
        <div className="mt-8">
          ...
          <div className="border-t py-4">
            ...
            <p className="text-gray-700 mt-2 text-sm">Absolutely great product!</p>
          </div>
        </div>
      </div>
    </div>
